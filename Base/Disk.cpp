    ...
    : Disk(std::move(stream), DiskType::SAD)
{
    ...
    if (stream->IsOpen())
    {
        stream->Read(m_data.data() + sizeof(sh), uDiskSize - sizeof(sh));
        stream->Close();
    }
}
    ...
    : Disk(std::move(stream), DiskType::EDSK), m_pSector(nullptr), m_pbData(nullptr)
{
    ...
    for (uint8_t cyl = 0; cyl < m_uTracks; cyl++)
    {
        for (uint8_t head = 0; head < m_uSides; head++)
        {
            ...
            if (pt && (stream->Read(pt, size) != size || (pt->bRate && pt->bRate != 1) || (pt->bEncoding && pt->bEncoding != 1)))
            {
                delete[] pb;
                pt = nullptr;
                size = 0;
