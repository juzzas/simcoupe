...
INT_PTR CALLBACK NewDiskDlgProc(HWND hdlg_, UINT uMsg_, WPARAM wParam_, LPARAM lParam_)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDOK:
        {
            ...
            switch (nType)
            {
            case 0: pDisk = std::make_unique<CMGTDisk>(std::move(stream)); break;
            default:
            case 1: pDisk = std::make_unique<CEDSKDisk>(std::move(stream));  break;
            case 2: pDisk = std::make_unique<CMGTDisk>(std::move(stream), DOS_DISK_SECTORS); break;
            }
